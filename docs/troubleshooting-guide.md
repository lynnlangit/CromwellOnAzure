...
4) **The Institution**: This is an enterprise level deployment scenario for a large organization with multiple Subscriptions and independant user groups within an internal hierarchy. In this scenario, due to the independant nature of the work being done and the desire/need to track specific resource usage (for your own internal billing purposes) you will have ***completely independant*** deployments of Cromwell on Azure.  
   ...
   To deploy, you'll need to verify whether an existing Azure Batch Service already exists on your Subscription (to run Cromwell on Azure on the Subscription level), or within your Resource Group as described in the [Getting Started guide](INSERT LINK HERE), with appropriate [roles](INSERT LINK HERE) set. If Azure Batch service is not deployed on your Subscription (or if you have available Quota to create a new Batch servce- the default for most accounts is 1 Batch service/region), then simply follow the [Quick Start Guide](INSERT LINK HERE). If there is a standing Azure Batch service you're hooking into within your Subscription, simply follow the deployment recomendations outlined in [3], adding the appropriate flags for the deployment script.
